...
const MoviesManagement = () => {
  ...
  const fetchMovies = async () => {
    try {
      const res = await axiosInstance.get("/api/v1/movies/getAllMovie");
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axiosInstance.delete(`/api/v1/movies/delete/${id}`, {
        headers: {
  ...
  return (
    <div className="container mx-auto px-4 py-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
        <h1 className="text-2xl sm:text-3xl font-bold text-white">🎬 Movies Management</h1>
